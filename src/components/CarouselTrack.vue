...
export default {
  ...
  computed: {
    ...
    // Make an array of inactive slide indices
    inactiveSlides() {
      return Array.from({ length: this.uniqueSlidesCount }, (_, index) => index).filter(
        (index) => !this.activeSlides.includes(index),
      )
    },
  ...
  methods: {
    makeSlides() {
      return this.getSlideComponents().map((vnode, index) => {
        ...
        let { class: staticClass = '', style = {}, attrs = {} } = vnode.props || {}
        let key = vnode.key
        ...
        if (
          isPeekingClone &&
          ![this.leftPeekingSlideIndex, this.rightPeekingSlideIndex].includes(peekingIndex)
        ) {
          style.display = 'none'
        }
    ...
    getSlideComponents() {
      ...
      return [...defaultSlots, ...clonedSlots].filter((vnode) => !getSlotChildrenText(vnode))
    },
